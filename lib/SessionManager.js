...
class SessionManager {
  ...
  updateSessionJarvisMode(sessionId, jarvisMode) {
    const session = this.sessions.get(sessionId);
    if (session) {
      session.jarvisMode = jarvisMode;
      
      if (jarvisMode) {
        // Initialize Jarvis mode: planning mode + empty plan
        ...
        // Disable Jarvis mode: remove mode and plan
        delete session.mode;
        delete session.plan;
      ...
      this.saveSessions();
      console.log(`🔄 Session ${sessionId} Jarvis mode updated: ${jarvisMode}`);
      return true;
    }
  ...
  updateSessionPlan(sessionId, plan) {
    ...
    if (session) {
      session.plan = plan;
      this.saveSessions();
      console.log(`📋 Session ${sessionId} plan updated`);
      return true;
    }
    console.error(`❌ Session ${sessionId} not found for plan update`);
    return false;
  }
