...
class SessionManager {
  ...
  updateSessionWithClaude(sessionId, claudeSessionId) {
    // Check if we're changing an existing value
    const currentSession = this.db.getSession(sessionId);
    if (currentSession && currentSession.claudeSessionId && currentSession.claudeSessionId !== claudeSessionId) {
      console.log(`🔄 CLAUDE SESSION ID CHANGE: ${sessionId} ${currentSession.claudeSessionId} → ${claudeSessionId}`);
    }
