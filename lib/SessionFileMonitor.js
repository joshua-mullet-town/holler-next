  ...
  async processNewMessage(sessionId, message) {
    ...
    if (type === 'user') {
      ...
      if (msgContent?.role === 'user' && !message.toolUseResult) {
        console.log(`💬 User message detected in session ${sessionId}`);
      ...
      if (msgContent?.role === 'assistant') {
        console.log(`🤖 Claude response detected in session ${sessionId}`);
        
        // Debug: Show message content structure
        console.log(`🔍 COMPLETION DEBUG: Message content:`, {
          role: msgContent.role,
          stop_reason: msgContent.stop_reason,
          content_length: msgContent.content?.length || 0,
          has_content: !!msgContent.content,
          content_type: typeof msgContent.content
        });
        
        // Check if this is a complete text response (not tool usage)
        const hasToolUse = msgContent.content?.some(c => c.type === 'tool_use');
        const isComplete = msgContent.stop_reason === null && !hasToolUse;
        console.log(`🔍 COMPLETION DEBUG: isComplete = ${isComplete} (stop_reason: ${msgContent.stop_reason}, hasToolUse: ${hasToolUse})`);
        
        // Emit Stop event (response complete)
        this.emit('stop', {
          sessionId,
          message: msgContent,
          isComplete,
